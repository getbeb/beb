#!/bin/sh
# Install beb: detect the platform, fetch the latest release binary,
# verify its checksum, place it on PATH.
#
#   curl -fsSL https://getbeb.dev/install.sh | sh
#
# BEB_INSTALL_DIR overrides the destination (default ~/.local/bin).
set -eu

REPO=getbeb/beb
BIN_DIR=${BEB_INSTALL_DIR:-$HOME/.local/bin}
...
if command -v sha256sum >/dev/null 2>&1; then
    ...
    have=$(shasum -a 256 "$tmp/beb" | awk '{print $1}')
fi
want=$(awk -v f="beb-$target" '$2 == f { print $1 }' "$tmp/SHA256SUMS")
if [ -z "$want" ] || [ "$have" != "$want" ]; then
    echo "checksum mismatch for beb-$target; not installed" >&2
    exit 1
fi

mkdir -p "$BIN_DIR"
install -m 755 "$tmp/beb" "$BIN_DIR/beb"
echo "installed $BIN_DIR/beb"
...
command -v ssh-keygen >/dev/null 2>&1 ||
    echo "note: beb needs ssh-keygen (OpenSSH 8.2+) on PATH at runtime; install openssh" >&2
